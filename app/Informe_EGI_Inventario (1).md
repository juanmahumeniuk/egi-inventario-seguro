...
classDiagram
    ...
    Maquina "1" --> "1..*" PersonaMaquina : asignacion
    Maquina --> Aula
```
...
* **`id` (`BIGINT`):** Identificador único, clave primaria (`PK`) con propiedad autoincremental (`IDENTITY`).
* **`nombre` / `apellido` (`NVARCHAR(100)`):** Datos identificatorios del individuo en formato de texto de longitud variable.
* **`categoria` (`NVARCHAR(20)`):** Tipo de persona. Restringido mediante una restricción `CHECK` para validar los roles institucionales admitidos (ej. `'docente'`, `'alumno'`, `'tecnico'`).

**Maquina**
Representa cada computadora física inventariada en la institución.
* **`id` (`BIGINT`):** Identificador único, clave primaria (`PK`) con propiedad autoincremental (`IDENTITY`). Este ID se utiliza como referencia cruzada en MongoDB para recuperar el historial técnico y datos de hardware.
* **`numero_mesa` (`INT`):** Número entero que indica el banco o mesa dentro del espacio físico asignado.
* **`fecha_mantenimiento` (`DATE`):** Registro de la fecha del último mantenimiento.
* **`aula` (`Aula`):** Ubicación física del equipo. Está vinculada directamente a la enumeración `Aula` que restringe sus valores posibles.

**PersonaMaquina** *(Tabla de relación)*
Tabla de asociación que implementa la relación de muchos a muchos (N:M) entre `Persona` y `Maquina`, registrando las asignaciones temporales de los equipos.
* **`persona_id` (`BIGINT`):** Clave foránea (`FK`) que hace referencia a `Persona`. Parte de la clave primaria compuesta (`PK`).
* **`maquina_id` (`BIGINT`):** Clave foránea (`FK`) que hace referencia a `Maquina`. Parte de la clave primaria compuesta (`PK`).
* **`fecha_asignado` (`DATE`):** Fecha en la que se realizó y registró la asignación.
...
**Aula** *(Enumeración)*
Estructura que define y limita los valores posibles para la ubicación física de las máquinas a los laboratorios y aulas autorizadas:
* `AULA_1y2`
* `LABORATORIO_SO`
* `LABORATORIO_REDES`
* `AULA_4`
